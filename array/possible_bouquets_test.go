...
func TestMinDaysPossibleBouqets(t *testing.T) {
	t.Log(`
	...
	t.Run("Suite 1", func(t *testing.T) {
		...
		k := 1
		exp := 3
		assert.Equal(t, exp, RunMinDaysPossibleBouqets(bloomDay, m, k))
	})
...
func RunMinDaysPossibleBouqets(bloomDay []int, m int, k int) int {
	...
	for left <= right {
		...
		if isPossibleBouqets(bloomDay, m, k, mid) {
			ans = mid
			right = mid - 1
		} else {
			left = mid + 1
...
func isPossibleBouqets(bloomDay []int, m int, k int, day int) bool {
	consecutiveDay, bouqets := 0, 0
	for i := 0; i < len(bloomDay) && bouqets < m; i++ {
		if bloomDay[i] <= day {
			consecutiveDay++
			if k == consecutiveDay {
				consecutiveDay = 0
